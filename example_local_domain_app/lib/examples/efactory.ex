defmodule Examples.EFactory do
  ...
  def store_api_handler() do
    ...
    assert {:ok, func} = Anoma.LocalDomain.Storage.read_local(~k"/api_handler/constantly/get")
  ...
  def factorial_api_handler() do
    ...
    factorial_fn = fn
      ...
      n ->
        ...
        {:ok, result} = Anoma.LocalDomain.Scry.scry(~k"/anoma/local/foo/bar/factory/factorial/get/!n1")
        String.to_integer(n) * result
    end

    Factory.store("factorial", "get", factorial_fn)
    Anoma.LocalDomain.Scry.scry(~k"/anoma/local/foo/bar/factory/factorial/get/5")
  end
end
